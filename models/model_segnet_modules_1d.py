...
class ShareNet(nn.Module):
    ...
    def forward(self, x, weights=None, bn_mean=None, bn_var=None):
        if weights is None:
            ...
            x = self.relu(self.decoder_bn42(self.decoder_conv42(x)))

            # Stage 1d
            y = F.max_unpool2d(x, id1, kernel_size=2, stride=2)
            ...
            x = self.conv_layer_ff(x, weights, 'decoder_conv42.weight', 1, bn_mean, bn_var, 'decoder_bn42.running_mean', 'decoder_bn42.running_var', 'decoder_bn42.weight', 'decoder_bn42.bias')
            y = F.max_unpool2d(x, id1, kernel_size=2, stride=2)
...
class BranchNet(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.bn51(self.conv52(x)))
