...
class SegNet(nn.Module):
    def __init__(self, output_dim=[13,1,3], num_channels=[64,128,256,512,512]):
        ...
        for i in range(self.num_task):
            ...
            layers.append(nn.ReLU(inplace=True))
            self.task_3d.append(nn.Sequential(*layers))
    ...
    def forward(self, x):
        ...
        y = [0] * self.num_task
        for i in range(self.num_task):
            y[i] = self.task_3d[i](x)

         # Stage 2d
        for i in range(self.num_task):
            y[i] = F.max_unpool2d(y[i], id2, kernel_size=2, stride=2)
            y[i] = self.task_2d[i](y[i])
