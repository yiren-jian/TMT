...
class BranchNet(nn.Module):
    def __init__(self, output_dim=13, num_channels=[32,64,128,256,256]):
        ...
        self.deconv1 = deconv3x3(in_planes=num_channels[4], out_planes=num_channels[4], stride=2)
        self.bn1 = nn.BatchNorm2d(num_channels[4])
        self.conv11 = conv3x3(in_planes=num_channels[4], out_planes=num_channels[4])
        self.bn11 = nn.BatchNorm2d(num_channels[4])
        self.conv12 = conv3x3(in_planes=num_channels[4], out_planes=num_channels[3])
        self.bn12 = nn.BatchNorm2d(num_channels[3])

        self.deconv2 = deconv3x3(in_planes=num_channels[3], out_planes=num_channels[3], stride=2)
        self.bn2 = nn.BatchNorm2d(num_channels[3])
        self.conv21 = conv3x3(in_planes=num_channels[3], out_planes=num_channels[3])
        self.bn21 = nn.BatchNorm2d(num_channels[3])
        self.conv22 = conv3x3(in_planes=num_channels[3], out_planes=num_channels[2])
        self.bn22 = nn.BatchNorm2d(num_channels[2])

        self.deconv3 = deconv3x3(in_planes=num_channels[2], out_planes=num_channels[2], stride=2)
        self.bn3 = nn.BatchNorm2d(num_channels[2])
        self.conv31 = conv3x3(in_planes=num_channels[2], out_planes=num_channels[2])
        self.bn31 = nn.BatchNorm2d(num_channels[2])
        self.conv32 = conv3x3(in_planes=num_channels[2], out_planes=num_channels[1])
        self.bn32 = nn.BatchNorm2d(num_channels[1])

        self.deconv4 = deconv3x3(in_planes=num_channels[1], out_planes=num_channels[1], stride=2)
        self.bn4 = nn.BatchNorm2d(num_channels[1])
        self.conv41 = conv3x3(in_planes=num_channels[1], out_planes=num_channels[0])
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn31(self.conv31(x)))
